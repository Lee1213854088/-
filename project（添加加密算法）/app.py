...
from cry import aes_decrypt, aes_encrypt
import config, pymysql, requests, json, base64, time
...
def deal(id, salesman, buyer, number):
    ...
    id = aes_encrypt(aes_key, id)
    salesman = aes_encrypt(aes_key, salesman)
    buyer = aes_encrypt(aes_key, buyer)
    number = aes_encrypt(aes_key, str(number))
    ...
    print(result)
    result_ = aes_decrypt(aes_key, result[0])
    buyer_money = float(result_)  # 注意aes_decry
    number = float(aes_decrypt(aes_key, number))
    if buyer_money < number:
        ...
        result = cursor.fetchone()
        result_ = aes_decrypt(aes_key, result[0])
        salesman_money = float(result_)   # 注意aes_decry
        buyer_money -= number
        salesman_money += number
        sql = "UPDATE accounts SET money = %s WHERE username = %s"
        new_buyer_money = aes_encrypt(aes_key, str(round(buyer_money, 2)))
        execute(sql, new_buyer_money, buyer)
        sql = "UPDATE accounts SET money = %s WHERE username = %s"
        new_salesman_money = aes_encrypt(aes_key, str(round(salesman_money, 2)))
        execute(sql, new_salesman_money, salesman)
...
def info_search(database):
    ...
    if username and id:
        q = q.where(database.username == aes_encrypt(aes_key, username))
        q = q.where(database.id == aes_encrypt(aes_key, id))
    if username:
        q = q.where(database.username == aes_encrypt(aes_key, username))
    elif id:
        q = q.where(database.id == aes_encrypt(aes_key, id))
    ...
    return {
        ...
        'data': [
            {
                'id': aes_decrypt(aes_key, item.id),
                'username': aes_decrypt(aes_key, item.username),
                'password': aes_decrypt(aes_key, item.password),
            } for item in items
        ]
    }
...
def feedback_func(username, html):
    if request.method == 'GET':
        ...
        advices = aes_encrypt(aes_key, advices)
        ...
        new_id = aes_encrypt(aes_key, str(new_id))
        username_ = aes_encrypt(aes_key, username_)
...
def login_func(sql, session_name, url, return_template):
    if request.method == 'POST':
        ...
        username = aes_encrypt(aes_key, username)
        password = request.form.get('password')
        password = aes_encrypt(aes_key, password)
        res = execute(sql, username, password)
        if res:
            username = aes_decrypt(aes_key, username)
...
def post_data(database):
    ...
    for keys, value in data.items():
        data[keys] = aes_encrypt(aes_key, str(value))
    info = database()
    info.update(data)
    try:
        info.save()
...
def modify(database, id):
    data = request.get_json()
    info = db.get_or_404(database, aes_encrypt(aes_key, str(id)))
    print(info)
    # id不可重复
    # sql_check = f"SELECT * FROM {db} WHERE id = %s"
    # res = execute(sql_check, aes_encrypt(aes_key, data['id']))
    # print(res)
    # if res:
    #     return {
    #         'code': 1,
    #         'msg': 'id已存在'
    #     }
    # 对data中的每个键值对应的值执行crypto函数
    for keys, value in data.items():
        data[keys] = aes_encrypt(aes_key, str(value))
    # student = StudentORM.query.get(sid)
    # print(data)
    info.update(data)
    # sql = f"UPDATE {database} SET {arg1} = %s WHERE username = %s"
    try:
        info.save()
    except Exception as e:
        return {
...
def delete(database, id):
    info: database = db.get_or_404(database, aes_encrypt(aes_key, str(id)))
    try:
...
def update_user_data(loggedInUsername, newUsername, newPassword):
    ...
    loggedInUsername = aes_encrypt(aes_key, loggedInUsername)
...
def transfer():
    if 'username' not in session:
        ...
        res = execute(sql, aes_encrypt(aes_key, loggedInUsername))
        if res:
...
def transfer_deal():
    ...
    password = request.form.get('password')
    account_send = aes_encrypt(aes_key, account_send)
    account_receive = aes_encrypt(aes_key, account_receive)
    conn, cursor = get_conn()
    sql = "SELECT pay_pw FROM pay WHERE username = %s"
    cursor.execute(sql, (account_send,))
    result = cursor.fetchone()
    if aes_decrypt(aes_key, str(result[0])) == str(password):
        sql = "SELECT money FROM accounts WHERE username = %s"
        cursor.execute(sql, (account_send,))
        result = cursor.fetchone()
        sender_money = float(aes_decrypt(aes_key, str(result[0])))
        sql = "SELECT id FROM accounts WHERE username= %s"
        ...
        try:
            ...
            if res:
                ...
                result = cursor.fetchone()
                receivers_money = float(aes_decrypt(aes_key, str(result[0])))
                print(receivers_money)
                print(sender_money)
                # 对应加减账户余额
                sender_money -= money
                receivers_money += money
                print(round(receivers_money, 2))
                print(round(sender_money, 2))
                sql = "UPDATE accounts SET money = %s WHERE username = %s"
                execute(sql, aes_encrypt(aes_key, str(round(sender_money, 2))), account_send)
                sql = "UPDATE accounts SET money = %s WHERE username = %s"
                execute(sql, aes_encrypt(aes_key, str(round(receivers_money, 2))), account_receive)
                # 写入记录
                sql = "SELECT COUNT(*) FROM test_deal_info"
                count_result = execute(sql)
                id = count_result[0][0] + 1
                sql_insert_user = "INSERT INTO test_deal_info (id, salesman, buyer, number) VALUES (%s, %s, %s, %s)"
                execute(sql_insert_user, aes_encrypt(aes_key, str(id)), account_receive, account_send, aes_encrypt(aes_key, str(money)))  # 加入数据库
                close_conn(conn, cursor)
                return {'code': 0}  # 成功
            else:
                close_conn(conn, cursor)
...
def user_manage():
    ...
    if user_data:
        username = aes_decrypt(aes_key, user_data['username'])
        password = aes_decrypt(aes_key, user_data['password'])
    else:
        username = ""
        password = ""
...
def update_user():
    ...
    newUsername = aes_encrypt(aes_key, newUsername)
    newPassword = aes_encrypt(aes_key, newPassword)
    # 检查新用户名是否唯一
...
def user_account():
    ...
    try:
        ...
        if username is None or money is None:
            return render_template('user_account_none.html')
        user_data = {'username': username, 'money': money}
        if user_data:
            username = aes_decrypt(aes_key, user_data['username'])
            money = aes_decrypt(aes_key, user_data['money'])
        else:
            username = ""
            money = ""
...
def pay():
    ...
    loggedInUsername = aes_decrypt(aes_key, loggedInUsername)
    result = cursor.fetchone()
    close_conn(conn, cursor)
    try:
        ...
        if user_data:
            username = aes_decrypt(aes_key, user_data['username'])
            pay_pw = aes_decrypt(aes_key, user_data['pay_pw'])
        else:
            username = ""
            pay_pw = ""
...
def update_pay():
    ...
    if str.isdigit(newpay_pw) is False:
        return {'code': -1, 'msg': '密码应为数字，不可为空且长度应为6位'}
    # 更新数据库中的支付密码
    sql_update = "UPDATE pay SET pay_pw = %s WHERE username = %s"
    execute(sql_update, aes_encrypt(aes_key, newpay_pw), aes_encrypt(aes_key, loggedInUsername))
...
def info_for_deal():
    ...
    if salesman and id:
        q = q.where(test_deal_info.salesman == aes_encrypt(aes_key, salesman))
        q = q.where(test_deal_info.id == aes_encrypt(aes_key, id))
    if salesman:
        q = q.where(test_deal_info.salesman == aes_encrypt(aes_key, salesman))
    elif id:
        q = q.where(test_deal_info.id == aes_encrypt(aes_key, id))
    ...
    if items is None:
        ...
        return {
            ...
            'data': [
                {
                    'id': aes_decrypt(aes_key, item.id),
                    'salesman': aes_decrypt(aes_key, item.salesman),
                    'buyer': aes_decrypt(aes_key, item.buyer),
                    'number': aes_decrypt(aes_key, item.number)
                }
                for item in items
            ]
        }
...
def info_for_account():
    ...
    id = request.args.get('id')
    if username and id:
        q = q.where(accounts.username == aes_encrypt(aes_key, username))
        q = q.where(accounts.id == aes_encrypt(aes_key, id))
    if username:
        q = q.where(accounts.username == aes_encrypt(aes_key, username))
    elif id:
        q = q.where(accounts.id == aes_encrypt(aes_key, id))
        ...
        'data': [
            {
                'id': aes_decrypt(aes_key, item.id),
                'username': aes_decrypt(aes_key, item.username),
                'money': aes_decrypt(aes_key, item.money),
            } for item in items
        ]
    }
...
def info_for_user_deal():
    ...
    salesman = request.args.get('salesman')
    loggedInUsername = session.get('username')  # 从 session 中获取管理员用户名
    q = q.where(test_deal_info.buyer == aes_encrypt(aes_key, loggedInUsername))
    if salesman and id:
        q = q.where(test_deal_info.salesman == aes_encrypt(aes_key, salesman))
        q = q.where(test_deal_info.id == aes_encrypt(aes_key, id))
    if salesman:
...
def pay_connect():
    if request.method == 'POST':
        ...
        if id_ is not None:
            ...
            result = execute(sql_check_user, aes_encrypt(aes_key, buyer_))
            print(result[0][0])
            if str(result[0][0]) == aes_encrypt(aes_key, pay_pw):
                id = id_
                salesman = salesman_
                buyer = buyer_
                money = money_
